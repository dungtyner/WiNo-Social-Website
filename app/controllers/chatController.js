...
class chatController {
  async getListBoxChat(req, res) {
    console.log('getListBoxChat() running');
    var listBoxChat = [];
    var slug_personal;

    console.log("SESSION LOGIN", req.session.loginEd);
    var result = await getDataAccount_byID(req.session.loginEd);
    if (result) {
      slug_personal = result.slug_personal;
      // console.log(result.list_id_box_chat);
      ...
      listBoxChat.then(async(data) => {
          ...
          data.map(async (el, idx) => {
            if(el[0].lastSessionMessage)
            {
              ...
              var is =(dataBoxChat.members.some(el=>{
                return (el.slug_member==slug_personal&&el.startContent&&el.startContent>0&&el.startContent==dataBoxChat.content_messages.length-1) }))
                if(!is)
                {
        ...
        list_boxChat.forEach((box_chat,idx)=>{
          ...
          else
          {
            if(true)
            {
              var i  = idx-1;
            var tmp = box_chat
            // console.log(new Date(list_boxChat[i].lastSessionMessage.time_send).getTime(),new Date(tmp.lastSessionMessage.time_send).getTime());
            while (i>=0 && tmp.lastSessionMessage && list_boxChat[i].lastSessionMessage && new Date(list_boxChat[i].lastSessionMessage.time_send).getTime()<new Date(tmp.lastSessionMessage.time_send).getTime())
            {
        ...
        // console.log(list_boxChat);
        // console.log(`-----------------`);
        // list_boxChat.forEach(box_chat=>{
        //   console.log('box_chat after',new Date(box_chat.lastSessionMessage.time_send));
        // })
  ...
  async request_getDetailChat(req, res) {
    ...
    result.members.forEach(member=>{
      if(member.slug_member==dataAccount.slug_personal && member.startContent>0)
      {
        result.content_messages = result.content_messages.filter((content_message,idx)=>idx>member.startContent)
      }
    })
    res.send({
      result,
  ...
  async addMessage({ idChat, value_content_sessionMessage }) {
    ...
    BoxChat.findOne({ _id: idChat }).then(async (data) => {
      ...
      data.members = Promise.all(
        await data.members.map(async (member, idx) => {
    ...
    DriveController.searchFolderWithPath(path)
      ...
      .then(async (result) => {
        ...
        if(!result)
        {
          ...
          result = await DriveController.createFolder({name:req.body.idChat,idFolderParent:folder_parent.id})
        }
  ...
  async getDetailChat_withID(_id, idAccount) {
  ...
  async load_roomSocket_Chat(account) {
    ...
    await nsp_chat.on("connect", async (socket) => {
      ...
      await account.list_id_box_chat.forEach(async (el, idx) => {
    ...
    await global.io
      ...
      .then((sockets) => {
        ...
        //       socket.join(`CHAT_${el}`);
        //   await  socket.on(
        //       `IN_${el}_PEOPLE_SENDING`,
        //       async (content_messages)=>{
        //       await  listener(content_messages,socket,el)
        //       }
        //   )
        //   await  socket.on(`IN_${el}_PEOPLE_TYPING`, (account) => {
    ...
    if (!tmp_session_mess.isShare) {
      ...
      files.forEach(async (file) => {
        ...
        if (file.type != "application") {
          ...
          var id_file = file.value.urlFile.substring(
            "https://drive.google.com/uc?export=view&id=".length,
  ...
  async request_removeChat(req,res)
  {
    ...
    box_chat.members.forEach((member,idx)=>{
      ...
      {
        box_chat.members[idx].startContent=box_chat.content_messages.length-1
      }
    })
    // console.log(box_chat.members);
...
var data_box_chats = await Promise.all(data_account.list_id_box_chat.map(async id_box_chat=>{
  // console.log(id_box_chat);
  return new chatController().getShortChat(await BoxChat.findOne({_id:id_box_chat}),req.session.loginEd);
}));
...
// ''.toLocaleLowerCase
  return data_box_chat.nameChat.toLocaleLowerCase().indexOf(req.query.keyword.toLocaleLowerCase())>-1
})
...
async request_modifyNameChat(req,res)
{
  ...
  box_chat.markModified('name_chat'),
  box_chat.save();
res.send({mess:'ok'})
}
}
