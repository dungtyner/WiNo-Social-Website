...
const { search } = require("../../routes/account");
  ...
  async SearchFolder_WithName_inFolder(nameFolder, folder) {
    var query = {
      q: `
        ...
        ${
            folder != null
              ? ` and '${folder.id}' in parents`:""} and mimeType = 'application/vnd.google-apps.folder'
        `,
    };
    try {
  ...
  async searchFolderWithPath(path)
{
    ...
    result = Promise.resolve(
        await arr_path.map(async(folder,idx)=>{
            ...
            {
                folder_parent = await  new DriveController().SearchFolder_WithName_inFolder(arr_path[idx],folder_parent)
