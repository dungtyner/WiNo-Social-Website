const {getDataAccount_byID,getDataAccount_bySlug} = require('../models/Account')
const AccountController = require('./accountController')

class FriendController{
    async req_getListRequestFriend(req,res)
    {
        res.send({result:await new FriendController().getListRequestFriend({
            id: req.session.loginEd
        })})
    }
    async req_getListResponseFriend(req,res)
    {
        res.send({result:await new FriendController().getListResponseFriend({
            id: req.session.loginEd
        })})
    }
    async req_getListFriend(req,res)
    {
        ...
        res.send({result: await new FriendController().getListFriend({
            id: req.session.loginEd
         })})
    }
    ...
    async requestAddFriend(req,res)
    {
        ...
        data_own_account.list_request_new_friend.push(new Obj_requestAddFriend({
        ...
        data_friend_account.notification.friend.response_new_friend.push(
            JSON.parse(JSON.stringify({time_request:response_new_friend.time_request,
                data_requester:data_own_account,isSeen:true
            }))
        )
        ...
        data_own_account.save()
            global.io.emit(`${data_friend_account.slug_personal}_UPDATE_COUNT_NOTIFICATION`,data_friend_account)
            global.io.emit(`${data_friend_account.slug_personal}_UPDATE_NOTIFICATION`,data_friend_account)
            global.io.emit(`${data_friend_account.slug_personal}_UPDATE_LIST_RESPONSE_NEW_FRIEND`,data_friend_account)
            global.io.emit(`${data_own_account.slug_personal}_UPDATE_REQUEST_ADD_NEW_FRIEND`,data_own_account)
        res.send({
            mess:'ok',
    ...
    async acceptAddNewFriend(req,res)
    {
        ...
        dataAccount_response.notification.friend.response_new_friend.forEach((item,idx)=>{
            console.log(item.data_requester.slug_personal,slug_requester);
            if(item.data_requester.slug_personal==slug_requester)
            {
                dataAccount_response.notification.friend.response_new_friend.splice(idx,1)
            }
        })
        ...
        global.io.emit(`${dataAccount_request.slug_personal}_UPDATE_LIST_SLUG_FRIEND`,dataAccount_request)
        global.io.emit(`${dataAccount_response.slug_personal}_UPDATE_LIST_SLUG_FRIEND`,dataAccount_response)
    ...
    async unFriend(req,res)
    {
        ...
        var dataAccount_unfriend = await getDataAccount_bySlug(req.body.slug_unfriend);
        ...
        console.log('dataAccount_unfriend.list_slug_friend',dataAccount_unfriend.list_slug_friend);
        console.log('dataAccount_own.list_slug_friend',dataAccount_own.list_slug_friend);
        ...
        global.io.emit(`${dataAccount_unfriend.slug_personal}_UPDATE_LIST_SLUG_FRIEND`,dataAccount_unfriend)
    ...
    async remove_requestAddFriend(req,res)
    {
        ...
        dataAccount_response.notification.friend.response_new_friend.forEach((item,idx)=>{
            ...
            if(item.data_requester.slug_personal==dataAccount_request.slug_personal)
            {
                dataAccount_response.notification.friend.response_new_friend.splice(idx,1)
            }
        })
        ...
        dataAccount_response.save();
        dataAccount_request.save();
        global.io.emit(`${dataAccount_response.slug_personal}_UPDATE_NOTIFICATION`,dataAccount_response)
        global.io.emit(`${dataAccount_request.slug_personal}_UPDATE_REQUEST_ADD_NEW_FRIEND`,dataAccount_request)
        global.io.emit(`${dataAccount_response.slug_personal}_UPDATE_LIST_RESPONSE_NEW_FRIEND`,dataAccount_response)
        res.send({mess:'ok'})
    }
