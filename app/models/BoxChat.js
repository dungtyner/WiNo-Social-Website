...
module.exports={BoxChat:mongoose.model("BoxChat",BoxChat),Chat,getCountNotificationChat};
...
async function getCountNotificationChat({id_account})
{
    ...
    var tmp_count = await Promise.all( dataAccount.list_id_box_chat.map(async (element,idx) => { 
        var box_chats = await mongoose.model("BoxChat",BoxChat).find({_id:element})
            box_chats.forEach(box_chat=>{
                box_chat.members.forEach(member=>{
                    // console.log(member);
                    if(member.slug_member==dataAccount.slug_personal && member.notification==true)
                    {
